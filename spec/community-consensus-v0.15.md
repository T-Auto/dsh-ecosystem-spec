# Community Consensus v0.15

**Status:** Draft / Experimental  
**Scope:** dsh 社区插件互操作最低公共契约  
**Authority:** 社区 Draft；非 dsh 官方标准  
**Normative assets:** `schemas/`、`registry/registry-0.15.json`、`registry/permissions-0.1.json`、`conformance/fixtures/`、`conformance/tests/run.js`

> 本文是可执行的实验性 contract 草案，对齐社区 RFC v0.15（元协议内核 + Facet 对象模型 + MCP ContentBlock 对齐）。未达到 Candidate 前，不得宣称 Stable、官方认证或安全隔离。

## 1. 规范效力和术语

本版本定义 Community v0.15 core。RFC 0002、0003、0004、0005 的延期内容不是 core contract；它们不得被实现以“默认兼容”的名义偷偷加入。

规范性关键字：**MUST / 必须**、**MUST NOT / 禁止**、**SHOULD / 应**、**SHOULD NOT / 不应**、**MAY / 可以**。没有这些关键字的句子是说明性文字。

每一项可执行要求都有稳定 ID，并应在 `conformance/requirements-v0.15.json` 中映射到 schema、fixture、测试和失败展示。

## 2. Manifest

### C-001 唯一发现入口

插件包根目录 MUST 提供唯一的 `dsh-plugin.json`。宿主、市场和验证器 MUST NOT 使用其他文件名作为 Community v0.15 的发现入口；其他 manifest 文件只能属于非标准生态。
...
- `requires`：依赖的 Host capability，按 `requires.contracts` 引用契约坐标；
- `permissions`：请求的授权；
- `provides`：插件间 service 能力；v0.15 MUST NOT 提供；
- `contributes`：静态贡献元数据；
- `subscriptions`：订阅的事件。

v0.15 的 schema 拒绝 `requires.services` 和 `provides`。插件间 service 归 RFC 0003。
...
契约引用 MUST 使用类 Kubernetes 坐标 `apiVersion + kind`（如 `commands.dsh/v1alpha1` + `Command`）。坐标由 Registry 定案（§4），v0.15 使用社区 RFC 的示意坐标并保留 v0.1 平面名作为 legacy 别名。
...
Capability 和 event 的契约坐标 MUST 来自 `registry/registry-0.15.json`；权限名称 MUST 来自 `registry/permissions-0.1.json`。每个 contract MUST 有坐标、版本、schema 引用和不可变 SHA-256 schema hash。v0.1 平面名（`storage.local` / `commands` / `messages.observe`）保留为 legacy 别名，只用于迁移期解析，不得作为新声明的主要形式。

组织私有扩展 MUST 使用 `x-org.example.*` 命名空间，并在宿主兼容性展示中标为私有扩展。
...
- `compatible`：required 契约和所需授权均满足；
- `compatible_degraded`：required 满足，optional 缺失，且每个缺失的 optional 都声明了 fallback；
- `waiting_authorization`：契约可用但当前授权未授予；
- `rejected`：required 契约/schemaHash 不可用，或声明非法；
- `unknown`：引用的契约版本或 registry 版本无法判断。
...
**fallback 是 optional 引用的必填字段**：它声明缺失该 capability 时的降级行为。没有 fallback 的 optional 声明是非法声明（`INVALID_MANIFEST`），不进入协商——"可选"必须有书面的降级答案，否则宿主无从判断降级是否安全。（上游 v0.15 未定 fallback 的承载机制，本文按本地 TUI 收紧执行，待社区定案后对齐。）

**`unknown` 的触发条件只有两种**：(a) 引用的契约坐标 group+kind 存在于 registry、但请求的 apiVersion 版本不在 registry 中；(b) registry 本身的版本高于协商器支持的版本，协商结果无从判断。注意区分：引用的契约**坐标**（group 或 kind）不在 registry 属于非法声明（`INVALID_MANIFEST` → `rejected`），只有 version 层面的无法判断才是 `unknown`。
...
### C-040 `storage.dsh/v1alpha1` `LocalStorage`

该能力提供插件私有持久化，按 Component（分发包）隔离。调用者只能访问自身 Component 的 namespace；跨插件读写 MUST 拒绝。读写分别需要 `storage.local.read` 和 `storage.local.write`，默认拒绝、可撤销。

宿主 MUST 定义输入/输出、并发、quota、错误码、deactivate/uninstall/purge 行为。v0.15 实现至少必须做到：重复 cleanup 幂等、secret 不进入普通日志、grant 撤销后新调用立即失败。完整 API schema 由 registry 条目引用的 contract profile 补充。不提供跨插件共享：共享存储本质是插件间组合问题，归 RFC 0003。

### C-041 `commands.dsh/v1alpha1` `Command`

v0.15 只支持 flat action leaf：一个全局 command ID 对应一个 handler，不支持 command tree、交互式 prompt 或流式输出。command ID MUST 全局唯一；重复注册 MUST 拒绝后来的 contribution，并返回 `DUPLICATE_CONTRIBUTION_ID`，绝不能由加载顺序仲裁。
...
### C-042 `messages.dsh/v1alpha1` `MessageObserver`
...
payload 的消息内容 MUST 采用与 MCP ContentBlock 对齐的结构（v0.15 当前子集：`text` / `image` 两种 block）。精确字段边界以社区 v0.15 §9 第 3 问的定案为准；未定案前实现不得静默扩展 block 类型或字段。

## 7. Lifecycle

### C-050 确定性生命周期

v0.15 使用 eager activation，以 runtime generation 为作用域（无按需激活）：
...
标准注册效果 MUST 归属到具体 plugin ID 和 activation instance。Ledger MUST 符合 `schemas/effect-ledger-record.schema.json`，至少包含单调序号、时间、runtime generation、操作（`create` / `bind` / `replace` / `release` / `cleanup-failed`）、稳定 resource ID、结果和必要的 replacement relation。
...
v0.15 是 `trusted-in-process`。capability/permission 用于兼容性、授权提示、审计和治理，**不是技术安全边界**。插件代码在技术上可能绕过 Broker 并继承宿主进程权限。
...
插件自身 `version`｜`manifestVersion`（manifest 结构）｜`facets.host.apiVersion`（要求的 Host API 范围）｜**各领域契约坐标版本**（随 `apiVersion` 独立演进）｜宿主产品版本｜SDK 发布版本。

v0 阶段按"minor 可能 breaking"的实验规则明确标注。某个领域契约升级时，只有该契约和依赖它的插件需要动；内核、无关插件和宿主不用重新发版。

## 11. 明确延期

以下内容不属于 Community v0.15 core：`before-*` 修改/取消事件、完整 Runtime/Presentation/Invocation/Transport/Control 分层、command tree、plugin-to-plugin service、完整 provenance、按需激活、sandbox、跨端声明式 UI、`net.*`/`fs.*`/session write、market certification、lockfile/modpack。
