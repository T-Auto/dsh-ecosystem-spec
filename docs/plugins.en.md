...
> This document is synchronized from the dsh-TUI repository's `docs/plugins.md`; links to dsh-TUI internal files are relative to the dsh-TUI repository.
...
are covered by [Themes](https://github.com/ccch1mneyyy/dsh-TUI/blob/main/docs/themes.en.md) and the skill seam below.
...
- `base` (`light`/`dark`/`dark-ansi`) is the required source for uncovered
  ...
  key table lives in [`src/theme.ts`](https://github.com/ccch1mneyyy/dsh-TUI/blob/main/src/theme.ts).
- Theme files are treated as **untrusted input**: unknown keys and invalid
  ...
  tolerance.
- Full contract: [Themes](https://github.com/ccch1mneyyy/dsh-TUI/blob/main/docs/themes.en.md).
...
- Once your plugin is done, submit the link so the community can find you:
  - The core repo's [`docs/links.md`](https://github.com/ccch1mneyyy/dsh-TUI/blob/main/docs/links.md) (PR to `ccch1mneyyy/dsh-TUI`)
