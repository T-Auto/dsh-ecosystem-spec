...
本库兼容 `Community Consensus v0.15`及后续标准（元协议坐标 + Facet 对象模型 + MCP ContentBlock 对齐），但在有关`dsh-TUI`适配于扩展插件的兼容上做了更强制的约束，希望进入 `dsh-TUI` 启动方式的插件， `dsh-TUI` 维护的插件市场、推荐列表，应满足 TUI 当前声明的准入版本与验证要求。
...
本库会优先响应 dsh 官方发布的标准，其次响应 Community Consensus（当前 v0.15 Draft），最后则是为适配`dsh-TUI` 插件的私有标准。

## 边界

- `spec/community-consensus-v0.15.md`：Community v0.15 core contract；只以本文及其 normative assets 为准。
- `spec/tui-admission-v0.15.md`：TUI 市场准入 policy；只约束 TUI 生态，不倒灌 Community contract。
- `rfc/0002-0005`：延期或实验 RFC；不是 Community v0.15 core。
...
测试覆盖 manifest/Host Descriptor/event envelope/ledger/claim 的正反 fixture、registry hash 漂移检查、契约坐标解析（未知 group/kind 拒绝、未注册版本判 unknown）、required/optional 协商、授权等待、授权后通过、重复 command ID 拒绝、client/worker facet 拒绝和 ContentBlock payload 校验。

## 文档地图

| 文档 | 作用 |
|---|---|
| `spec/community-consensus-v0.15.md` | Community v0.15 core contract |
| `spec/tui-admission-v0.15.md` | TUI 产品准入 profile |
| `PLUGIN-ADMISSION-CHECKLIST.md` | TUI 人机联合检查表 |
| `schemas/` | Manifest、Host、事件、ledger、claim schema |
| `registry/` | 真实 contract/permission registry 与 hash（坐标化） |
...
“Community Draft”“Deferred from v0.15”仅作为说明性标签，正式状态必须使用上述枚举。日期字段由发布流程维护，本仓库不以日期缺失阻断当前实验实现。
